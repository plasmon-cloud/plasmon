...
function tokenize(source: string): Token[] {
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    if (UNSUPPORTED_UNQUOTED.has(char)) {
      throw new Error(`.cmd experiment does not yet support shell operator ${JSON.stringify(char)}; use .run for this expression`);
    }
    word += char;
    wordStarted = true;
  }
...
function parseLine(tokens: readonly Token[]): CmdPipeline | null {
  ...
  for (const token of tokens) {
    ...
    if (afterRedirect) throw new Error(".cmd experiment requires stdout redirection to be the final expression");
    if (token.kind === "word") {
    ...
    if (token.kind === "redirect") {
      if (stdoutPath !== undefined) throw new Error(".cmd experiment supports one stdout redirection per pipeline");
...
/** Small local parser used until the mvdan/sh browser asset is packaged locally. */
