...
describe(".cmd v1 parser regression matrix", () => {
  ...
  test("rejects every intentionally deferred unquoted shell operator", async () => {
    const parser = new SimpleCmdParser();
    for (const operator of [";", "&", "<", "`", "$", "(", ")", "{", "}", "*"]) {
      await expect(parser.parse(`echo before ${operator} after`)).rejects.toThrow("does not yet support shell operator");
    }
  });

  test("allows deferred operator characters when they are quoted literals", async () => {
...
describe(".cmd v1 path and cwd behavior", () => {
...
describe(".cmd v1 command option and error matrix", () => {
