...
Monaco worker executables are packaged beneath the canonical `/System/Program Files/MonacoEditor` runtime root. That Program Files subtree is the sole logical runtime authority, not an application-installation registry. Ordinary browser contexts construct module Workers from that path directly. Neutron's application frame intentionally has an opaque origin, so a Worker created there cannot perform a second HTTP module load across the sandbox boundary. Packaged builds therefore generate one classic `runtime/monaco/worker-sources.js` transport before the application build. It contains the same bundled worker bytes as inert strings, is preloaded by `index.html`, and is used only to materialize a `blob:` module Worker inside the opaque frame. Package tests require every transported source to be byte-identical to its canonical Program Files worker; the obsolete per-worker HTTP mirror and retired top-level `monaco-workers/` path are not packaged. No transport owns Monaco lifecycle, model identity, document state, or Program Files reconciliation.
...
The #89 packaged worker acceptance separately runs in installed Chromium and Firefox. Package coverage proves the preloaded browser transport source is byte-identical to the canonical Program Files workers and that the obsolete HTTP mirror is absent; browser coverage verifies the installed transport preload, opaque-origin `blob:` module-Worker construction, real editor/TypeScript worker message exchange, zero worker errors/fallback warnings, and strict BrowserHealth during worker startup.
