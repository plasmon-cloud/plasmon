...
- `model.ts` — selection, marquee geometry, refresh gating, rename helpers, clipboard state, and canonical ordered directory-drop mutation with optional lifecycle observation;
- `operation-state.ts` — the small injectable FileManager operation lifecycle vocabulary for import/paste/drag-move status, item counts, current item where the owning workflow can report it truthfully, partial failures, and duplicate-start protection;
...
- `drag-preview.ts` — deterministic translation from source-entry geometry plus pointer delta into the drag-ghost rectangle, and presentation wording for a canonical operation/target;
...
### Drag preview and directory drop

An active FileManager resource drag has one canonical interaction state and one presentation of that state. The browser adapter may render a detached top-level ghost so the drag remains visible over windows, but the ghost is not a second resource, target selector, placement controller, or filesystem command path.

For a single resource, the ghost preserves the rendered resource identity: icon/thumbnail and filename stay arranged like the source entry, with translucent treatment indicating preview state. Its rectangle is derived from the source entry's browser rectangle plus the same pointer delta supplied to Desktop repositioning. Desktop remains the authority for final NodeId-keyed placement and may legitimately clamp/reconcile a proposed position; absent such a canonical correction, the last ghost position and committed Desktop entry should be visually continuous.

Directory hover is derived from actual browser hit testing plus the canonical filesystem/drop authority. A directory entry in the source FileManager remains an ordinary target. An already-open FileManager in another window additionally exposes only its current directory's stable `NodeId` on its content surface; the source pointer adapter may use that NodeId as a browser hit candidate, but it must resolve the candidate through `FsService.stat()` and `validateDirectoryDrop()` before presenting it as active. This permits Desktop-to-open-Explorer and other cross-FileManager folder drops without introducing module-global drag state or moving filesystem policy into Windowing. A visible ordinary file entry blocks its containing FileManager surface, so dropping on a normal file is not silently reinterpreted as dropping into the folder behind it.

Contextual text such as `Move to Documents` is only a presentation of the canonical resolved target and operation. Releasing while that target is active must invoke the existing `moveNodesToDirectory()`/`FsService` path; the UI must not advertise a destination that the corresponding release will not use. The detached preview and feedback remain pointer-transparent so they cannot become the hit-test target themselves.
...
FileManager prepares a selected file's Blob when its context menu opens. The Download action stays disabled until that preparation completes, then performs the existing object-URL/anchor download synchronously in the user action. This preserves browser transient user activation across the asynchronous `FsService` read without changing filesystem authority, filename/MIME/cleanup behavior, or the Kernel app-frame sandbox contract. The legacy hackathon-core package profile detects that its hosted app frame cannot download and reports an actionable compatibility error before claiming success; standalone previews and profiles with Kernel download support retain the normal path. A packaged/browser test remains the authority for proving the resulting browser-owned download; deterministic tests cover the helper, compatibility, and preparation seams only.
...
Use fast tests for selection/range/marquee math, clipboard/collision naming, operation-state transitions and presentation, refresh ordering, command eligibility, activation routing, rename/create/import/delete/shortcut helpers, drag/drop decisions, drag-preview geometry and contextual feedback derivation, filesystem action outcomes, persisted view preferences, pure view-strategy/spatial navigation, deterministic FileManager render state, deterministic FileEntry render state, and deterministic shared-presentation mapping. Hidden-file presentation tests must exercise the filesystem list contract rather than duplicating hidden-name classification in FileManager tests. Cross-surface activation and ordinary-Delete tests should use the shared headless Plasmon environment so FileManager's production adapters exercise the real filesystem dispatcher/Trash authority, associations, process/window state, protection policy, and Neutron boundary.

Use RTL/browser tests only where DOM mechanics are material, including semantic Grid/List/Details strategy cutover through the production Explorer composition, the accessible running/result-status boundary for deliberately delayed import/paste/drag-move operations, pointer capture/drag, drag-ghost icon/name fidelity and rendered release geometry, real directory hit testing/context feedback, Desktop-to-already-open-folder cross-FileManager drops, keyboard routing/editable targets, file chooser/import, object-URL download behavior, focus/dialog/context-menu interaction, rendered List/Details geometry, bounded Desktop FileEntry rename geometry, bounded command discoverability such as Send to Desktop, and packaged visible workflows. Installed Plasmon-owned artwork paths require packaged-browser coverage because standalone rendering cannot prove the Neutron application mount.
