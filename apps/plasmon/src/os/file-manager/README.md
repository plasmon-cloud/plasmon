...
Directory hover is derived from actual FileManager hit testing and `directoryDropTargetId()`. Contextual text such as `Move to Documents` is only a presentation of that canonical target and operation. Releasing while that target is active must invoke the existing `moveNodesToDirectory()`/`FsService` path; the UI must not advertise a destination that the corresponding release will not use. The detached preview and feedback remain pointer-transparent so they cannot become the hit-test target themselves.
...
Use RTL/browser tests only where DOM mechanics are material, including semantic Grid/List/Details strategy cutover through the production Explorer composition, the accessible running/result-status boundary for deliberately delayed import/paste/drag-move operations, pointer capture/drag, drag-ghost icon/name fidelity and rendered release geometry, real directory hit testing/context feedback, keyboard routing/editable targets, file chooser/import, object-URL download behavior, focus/dialog/context-menu interaction, rendered List/Details geometry, bounded Desktop FileEntry rename geometry, bounded command discoverability such as Send to Desktop, and packaged visible workflows. Installed Plasmon-owned artwork paths require packaged-browser coverage because standalone rendering cannot prove the Neutron application mount.
