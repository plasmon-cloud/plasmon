...
`/System/Start Menu` remains the durable filesystem authority for Start. Explorer is the only former managed `System` application retained as a managed default Start shortcut; Settings and Properties are no longer managed Start defaults. Shell-created Start categories record their stable folder `NodeId` as durable Start-root provenance, and existing same-name directories are never retroactively adopted as managed. A retired managed `System` category is migrated only when that exact directory `NodeId` is already present in durable provenance and its canonical contents remain untouched. Current inventory retirement removes an exact ledger-backed root Settings/Properties default only when no retained former `System` directory makes that shortcut's prior parent ambiguous; if such a folder remains, reconciliation consumes the retired seed identity but preserves the root shortcut `NodeId` because it may be a user move. The historical v1 seed ledger records shortcut identities but not shortcut NodeIds or the former `System` directory `NodeId`, so pre-provenance ownership cannot be inferred from name, contents, shortcut shape, identity, or timestamps. User renames, moves, replacements, deletions, metadata/content customization, and ambiguous folders are preserved. Reconciliation remains duplicate-free and idempotent, and filesystem `/System` is unaffected.
