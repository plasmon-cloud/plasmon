...
`/System/Start Menu` remains the durable filesystem authority for Start. Default Settings, Explorer, and Properties shortcuts are seeded directly at that Start root rather than under a managed visible `System` category. Managed categories created by Shell reconciliation carry durable folder provenance, while an existing same-name directory is never retroactively adopted as managed. That provenance permits a retired category to migrate only while both its folder ownership and untouched canonical contents remain provable. Historical v1 Start metadata recorded seeded shortcut identities but not the stable `NodeId` of the former managed `System` directory, so pre-provenance legacy `System` folders remain in place rather than being inferred from shape. Reconciliation scans those shortcuts in place without creating root duplicates; user renames, moves, deletions, folders, metadata, and content remain untouched.
