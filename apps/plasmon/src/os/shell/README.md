...
- `shell-coordination.ts` — deterministic Shell-global one-flyout/context/taskbar-group coordination only;
...
`/System/Start Menu` remains the durable filesystem authority for Start. Explorer is the only former managed `System` application retained as a managed default Start shortcut; Settings and Properties are no longer managed Start defaults. Shell-created Start categories record their stable folder `NodeId` as durable Start-root provenance, and existing same-name directories are never retroactively adopted as managed. A retired managed `System` category is migrated only when that exact directory `NodeId` is already present in durable provenance and its canonical contents remain untouched. Current inventory retirement removes exact ledger-backed root Settings/Properties defaults unless their prior parent is genuinely ambiguous: a still-existing provenance-recorded former `System` folder is sufficient, while a pre-provenance directory named `System` creates ambiguity only when it still contains an untouched ledger-backed former-System seed whose identity is absent from the Start root. Folder name alone never suppresses retirement, so an unrelated user-created `System` directory remains untouched but does not keep stale managed root Settings/Properties defaults alive. In an ambiguous legacy-move case reconciliation consumes the retired seed identity but preserves the root shortcut `NodeId`, preventing later reclamation. The historical v1 seed ledger records shortcut identities but not shortcut NodeIds or the former `System` directory `NodeId`, so this legacy footprint is used only to avoid destructive shortcut retirement and never to authorize moving or deleting the unproven folder. User renames, moves, replacements, deletions, metadata/content customization, and ambiguous folders are preserved. Reconciliation remains duplicate-free and idempotent, and filesystem `/System` is unaffected.
