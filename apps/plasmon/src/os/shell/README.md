...
`/System/Start Menu` remains the durable filesystem authority for Start. Default Settings, Explorer, and Properties shortcuts are seeded directly at that Start root rather than under a managed visible `System` category. Historical Start metadata records seeded shortcut identities but not the stable `NodeId` of the former managed `System` directory, so legacy `System` folders cannot be proven system-owned and are preserved in place rather than migrated by shape. This fail-closed preservation is the upgrade contract for pre-provenance installs: reconciliation scans those shortcuts in place, creates no root duplicates, and never deletes the folder merely because its shape resembles the retired default. Fresh reconciliation no longer creates that category, and user renames, moves, deletions, folders, and content remain untouched.
