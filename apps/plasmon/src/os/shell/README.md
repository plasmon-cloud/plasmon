...
`/System/Start Menu` remains the durable filesystem authority for Start. Default Settings, Explorer, and Properties shortcuts are seeded directly at that Start root rather than under a managed visible `System` category. Categories created by Shell reconciliation record their stable folder `NodeId` as durable Start-root provenance; an existing same-name directory is never retroactively adopted as managed. A retired `System` category is migrated only when that exact directory `NodeId` is already present in the provenance record and its canonical contents remain untouched. The historical v1 seed ledger records shortcut identities but not the former `System` directory `NodeId`, so pre-provenance folders cannot be proven system-owned from name, contents, shortcut identity, or timestamps and remain in place. User renames, moves, replacements, deletions, metadata/content customization, and ambiguous folders are preserved. Reconciliation remains duplicate-free and idempotent, and filesystem `/System` is unaffected.
