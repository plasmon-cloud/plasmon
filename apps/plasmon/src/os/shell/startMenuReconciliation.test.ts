...
import type { ExternalElement, NativeAppDefinition } from "../contracts/index.ts";
import {
  parseStartShortcut,
  reconcileStartMenu,
  START_MENU_PATH,
...
const neutronElement: ExternalElement = {
  id: "start-reconciliation-review",
  name: "Start Reconciliation Review",
  description: "Independent Start reconciliation fixture.",
  version: 1,
  tiles: [{ id: "main", title: "Start Reconciliation Review" }],
  running: "no",
};
...
test("malformed managed category is bounded, idempotent, and recoverable", async () => {
  ...
  try {
    ...
    const elementShortcut = await environment.node(`${START_MENU_PATH}/Neutron/Start Reconciliation Review`);
