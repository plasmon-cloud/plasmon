...
- preserves the first/unrelated valid owner during repair;
- ignores inactive persisted records as visible occupancy so Trash metadata may remain available for a future restore;
- recomposes deterministically when workspace geometry changes.

It does **not** inspect filesystem resource semantics, choose handlers, classify or render resources, perform Trash/restore operations, or own browser drag/drop commands.

`Desktop.tsx` remains the persistence/composition adapter. It measures usable browser geometry, supplies stable NodeIds and persisted metadata to the controller, renders/persists the resolved result, and translates explicit drag deltas through the pure drag-input adapter. `FileManager` continues to own shared file interaction and browser pointer mechanics; it does not define a second placement algorithm.
...
Use Bun for placement allocation, collision repair, bounds validation, resize/recomposition, NodeId stability, and explicit drag-position translation. Use real-browser tests only for claims requiring layout, such as proving rendered Desktop entries apply their authoritative `left`/`top` positions. Filesystem and Trash behavior remain protected at their existing authority layers.
