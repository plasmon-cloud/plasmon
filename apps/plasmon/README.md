...
- **Runtime configuration** — an orthogonal, declarative selection of optional heavyweight capabilities such as js-dos, EmulatorJS, and associated pinned content. #370 owns the durable runtime-delivery/configuration mechanism. A custom runtime configuration is data/configuration, not a new package tier or source-code fork.
...
Heavyweight runtime delivery remains separately owned by #370. Slim remains runtime-free; Base starts without heavyweight runtime payloads; Demo/custom preparations may opt into approved pinned runtimes without redefining either package tier.
