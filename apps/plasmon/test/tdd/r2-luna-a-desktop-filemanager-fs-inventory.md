...
|---|---|---|---|---|---|---|---|---|
| #44 | FileManager + Fs shortcut primitive | no PR | `issue-44-closure-audit.md` | VERIFIED CORE GREEN / INCOMPLETE ACCEPTANCE | #31 for open, already integrated | no honest new RED | packaged discoverability optional | `src/os/file-manager/create-shortcut.test.tsx`, `src/os/fs/desktopCore.test.ts`, `test/refactorGuards.test.ts`, `test/resourceOpenCrossSurface.test.ts` |
| #45 | Native Recycle Bin surface | no PR observed | Trash/restore matrices only; cross-lane surface | WAIT FOR IMPLEMENTATION / CROSS-SURFACE | Trash core integrated; native surface ownership must be assigned | no competing packet | future packaged surface | `trashLifecycle.test.ts` proves core only; surface closure remains |
...
| #78 | Cross-surface shortcut lifecycle | D/cross-surface ownership | canonical Issue is blocked label; Phase-1 NodeId evidence | WAIT FOR DEPENDENCY / CROSS-SURFACE | #31/#44/#51 and free D ownership | no Luna-A competing packet | packaged optional | `test/refactorGuards.test.ts` covers much of lifecycle; D must promote composed acceptance |
| #82 | managed-root bootstrap | D/cross-surface ownership | filesystem mutation matrix only | WAIT FOR CROSS-SURFACE OWNER | managed roots/reconciliation | no Luna-A packet | package/composition if required | `test/managedRootBootstrap.test.ts`, `src/os/fs/desktopCore.test.ts` |
...
| #92 | FileManager drag-move operation state | Luna-A future dependency packet | `issue-92-preserve-change-unspecified.md`, `issue-92-operation-model-consumption.md` | WAIT FOR DEPENDENCY | #65 must integrate; no #92 owner may exist | not yet truthful | no; deterministic model first | future accepted #65 vocabulary + #92 tests |
...
| #172 | Desktop placement + Trash restore composition | no PR; #192 integrated | existing composed test, closure audit, post-192 audit | VERIFIED PRODUCT RED ON STALE TDD BASE / WAIT FOR STAGING REFRESH | integrated #192 source absent in this worktree | yes against stale source; not valid integrated verdict | browser not needed unless adapter geometry | `issue-172.composed.red.test.ts` must run against integrated #192; Trash tests already green |
