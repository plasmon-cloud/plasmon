...
## Exact-head promotion audit — #208/#65 and #210/#51

The previous gap rows were based on an older snapshot and are superseded by the exact-head audit committed on the Luna-D branch.

### #208 / #65 — PROMOTION ACCEPTED

At `665670102efd63bbc766c5a51f1b24fcace2ced5`, `operation-state.test.ts` proves success, mixed failure, and duplicate-start rejection. `FileManager.tsx` begins one item-level operation, reports delayed import current item/count, continues after per-file failure, retains successful files, surfaces an alert, and clears status after settlement. Paste uses the same model and exposes only truthful `Pasting N item(s)…` state; it does not invent bytes or per-item copy progress. The `pasteClipboardCollisionAware` and `importFileIntoFs` helpers remain the FsService mutation/collision/identity authority. Exact-head CI run `31704920900` passed Fast Bun; Kernel, packaged specialist, and packaged smoke also passed. Supplemental two-file delayed import/partial-failure/duplicate-trigger assertions passed against the detached exact head during this audit; no RED was staged because the deterministic contract is implemented.

### #210 / #51 — PROMOTION ACCEPTED

At `b7e5a52d123d847cce98aea3e0aef2dfce20b392`, `send-to-desktop.test.ts` proves canonical NodeId target identity, unchanged source parent/identity, repeated collision naming, protected system source, stale target, unavailable `/Desktop`, and no partial state. `create-shortcut.ts` imports and delegates serialization/naming to the existing `createShortcut()` primitive; the adapter resolves `/Desktop` and performs no source move/copy. The RTL journey proves the eligible command is discoverable and creates the Desktop shortcut. Exact-head local helper/RTL execution passed with the canonical happy-dom preload; all exact-head PR checks passed (Kernel `31706878739`, Fast Bun `31706878744`, persistence `31706878721`, specialist `31706878719`, smoke `31706878736`). No RED was staged because the repaired deterministic contract is covered by equivalent ordinary tests.
