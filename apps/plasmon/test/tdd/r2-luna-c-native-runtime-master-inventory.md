...
| #64 | open | js-dos progress | FsService + js-dos engine adapter | no eligible owner | missing product/runtime seam packet | save packets | shipped engine save API/explicit boundary absent; identity/artifact/browser restore | yes | BLOCKED — MISSING JS-DOS SAVE/EXPORT/IMPORT SEAM |
...
| #79 | open | document close composition | Process/Windowing + Native Apps | Luna-C reconciliation | existing process/document/headless tests | `issue-79-83-reconciliation.md` | one production headless workflow already covered; visible close UI remains browser boundary | no | ALREADY GREEN / RECON COMPLETE |
| #83 | open/blocked | runtime selection | Association/OpenService | Luna-C reconciliation | existing runtime/headless tests | `issue-79-83-reconciliation.md` | composed two-runtime routes already covered; engine startup remains browser boundary | no | ALREADY GREEN / RECON COMPLETE |
...
| #113 | open | Text editor UX | Text + Monaco | Luna-C claimed | final implementor packet; category-A RTL gap | full matrix | missing title/commands/minimap; canonical Happy DOM cannot mount truthful Monaco CSS runtime | partly | FINAL IMPLEMENTOR PACKET READY + EXACT BROWSER REMAINDER |
| #114 | open | Markdown UX/formatter | Markdown + Monaco host | Luna-C claimed | final implementor packet; category-A RTL gap | full matrix | formatter/commands absent; canonical Happy DOM cannot mount truthful Monaco CSS runtime | partly | FINAL IMPLEMENTOR PACKET READY + EXACT BROWSER REMAINDER |
...
| #123 | open | game artwork | shared ResourcePresentation | Luna-C claimed | missing product-contract packet | artwork packets | #190 seam exists but canonical game-artwork metadata contract absent | yes/manual | BLOCKED — ARTWORK CONTRACT UNSPECIFIED |
| #124 | open/blocked | save screenshots | #64 save artifact + Visual | Luna-C claimed | dependency packet | preview packets | #64 save representation and explicit save boundary absent | yes | BLOCKED — DEPENDS ON #64 SAVE ARTIFACT |
