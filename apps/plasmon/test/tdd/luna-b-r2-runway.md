...
| #88 | **COMPLETE / NO IMPLEMENTATION REQUIRED** | `.red/issue-88.characterization.test.ts` → `src/os/shell/runtimeOnlyInventory.test.ts` | historical `tdd/88-red` gate is subsumed by 4 passing tests/35 assertions; branch safe for later deletion |
...
| #117 | **GREEN IN R2 — RED CONSUMED** | `src/os/windowing/placement.test.ts` + `test/issue-117-window-placement.test.ts` | durable geometry belongs to Windowing/Fs composition, never Shell; successor awaits durable flush |
| #118 | **GREEN IN R2 — RED CONSUMED** | `src/os/shell/taskbar.test.ts`, `taskbarMember.test.ts`, `taskbarPresentation.test.ts` + packaged group/context acceptance | ProcessId/WindowId remain member identities; no taskbar DB |
...
| #176 | **FINAL IMPLEMENTOR PACKET READY** | `.red/issue-176.red.ui.test.tsx` plus `issue-176-browser-adoption.md` | owned/specialized context menus claim events; editable and foreign boundaries remain unclaimed; current editable RED is executable |
