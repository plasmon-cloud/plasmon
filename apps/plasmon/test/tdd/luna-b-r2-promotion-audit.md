...
- staging baseline before this finalization pass: `f8f3474`

This audit checks promotion evidence rather than treating a finalized packet as implementation evidence. Focused release verification at `8cfb4d6` passed **75 tests / 299 assertions** across Shell, Windowing, Visual, runtime-only, taskbar, and document-close paths.

## Lane-B queue audit

There are **10** Lane-B queue entries. No Lane-B entry is currently `[~]`; the two genuine unconsumed REDs remain explicitly open below.

### Required finalization matrix

| Issue | Final TDD disposition | RED? | Gate | Product owner | Promotion |
|---|---|---|---|---|---|
| #61 | CHARACTERIZATION ONLY | NO | `issue-61.characterization.ui.test.tsx` | SOL 2 / #197 seam owner | N/A — CHARACTERIZATION |
| #63 | INTENTIONAL RED READY | YES | `.red/issue-63.red.ui.test.tsx` | SOL 2 | RED NOT YET CONSUMED |
| #72 | ALREADY GREEN | NO | `src/os/shell/taskbarPresentation.test.ts`, `taskbar.test.ts` | SOL 2 | N/A — ALREADY GREEN |
| #87 | ALREADY GREEN | NO | `gate3.test.ts`, `startMenuSystemMigration.test.ts`, `runtimeOnlyInventory.test.ts` | SOL 2 | N/A — ALREADY GREEN |
| #91 | INTENTIONAL RED READY | YES | `.red/issue-91.red.test.ts` | SOL 1 | RED NOT YET CONSUMED |
| #109 | ALREADY GREEN | NO | `src/os/visual/visual.components.test.tsx`, `visual.test.ts` | SOL 2 / Visual owner | N/A — ALREADY GREEN |
| #111 | ALREADY GREEN | NO | Visual component tests and Shell/refactor guards | #201 visual owner | N/A — ALREADY GREEN |
| #117 | INTENTIONAL RED READY | YES | `.red/issue-117.red.test.ts` → `test/issue-117-window-placement.test.ts` | SOL 2 | GREEN IN R2 |
| #118 | INTENTIONAL RED READY | YES | `.red/issue-118.red.test.ts` → taskbar grouping/member tests | SOL 2 | GREEN IN R2 |
| #119 | DEFERRED | NO | `issue-119.characterization.test.tsx`, `documentClose.test.ts` | Coordinator; no selected transient owner | N/A — DEFERRED |

#119 reopening trigger: a concrete native transient consumer must be selected with an owner/parent/focus contract; until then no Windowing transient authority or corrective RED may be fabricated.
...
| #63 | `luna-b-r2-runway.md`; `issue-63-browser-adoption.md` | YES | `.red/issue-63.red.ui.test.tsx`; original gate staged against `e815c463`; re-run at `8cfb4d6` still has 2 failures | none found in release or remote implementation refs | none; WindowManager MRU tests are not an Alt-Tab regression | NO | RED NOT YET CONSUMED |
...
| #91 | `luna-b-r2-runway.md` | YES | `.red/issue-91.red.test.ts`; refreshed RED staged against `61fac771`; re-run at `8cfb4d6` has 2 ordinary-cap failures and 1 safety-truncation pass | none; #174 Search projection work does not distinguish ordinary caps from safety truncation | none beyond existing cap test, which does not prove the distinction | NO | NO | RED NOT YET CONSUMED |
...
| #117 | `issue-117-acceptance-map.md` | YES, consumed | `.red/issue-117.red.test.ts`; original gate staged against `e815c463`; old copy still fails at `8cfb4d6` if it reconstructs before the durable flush boundary | #214 merge `4a21881`; replay `b5b077e` | `src/os/windowing/placement.test.ts` plus `test/issue-117-window-placement.test.ts`; successor explicitly awaits `windowPlacement.flush()` | YES — 4 placement assertions and integration test pass | YES | GREEN IN R2 |
| #118 | `issue-118-acceptance-map.md` | YES, consumed | `.red/issue-118.red.test.ts`; original gate staged against `e815c463`; re-run at `8cfb4d6` now passes | merged PR #237, merge `f3eb5f5`; grouping/member work includes `da4006b`, `9bb02ad`, `6117d21`, and later #198 reconciliation | `src/os/shell/taskbar.test.ts`, `taskbarMember.test.ts`, `taskbarPresentation.test.ts`, plus packaged group/context acceptance | YES — focused group tests pass | YES | GREEN IN R2 |
...
- **Base reproduced:** original staging `e815c46358f20b25fd5b15f6409adefa19dfcad3`; independently re-run against current release `8cfb4d68414b271303bd0afefdcac9dc8449c315`
...
- **PRESERVE:** `WindowManager.focusSnapshot()` / MRU is the sole focus history; minimized/closed exclusion, Process/Windowing lifecycle, and existing global keyboard ownership remain authoritative.
- **CHANGE:** Shell-level Alt-Tab invocation/presentation and deterministic cycling translated through WindowManager authority; no second focus history.
...
- **Base reproduced:** refreshed staging `61fac7716975d011bc05588f5fc6fcdbf335fa35`; independently re-run against release `8cfb4d68414b271303bd0afefdcac9dc8449c315`
...
These were prepared by this lane but are not among the 10 queue rows:
...
- **Current exact release SHA audited:** `8cfb4d68414b271303bd0afefdcac9dc8449c315`
