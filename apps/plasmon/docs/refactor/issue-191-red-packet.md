...
Adopted from Luna TDD-A packet `1e579bf` onto the canonical #191 implementation branch. These gates are production regressions now; the staging branch is not merged.
...
## Adopted regression files

- `src/os/file-manager/issue-191.characterization.test.ts`: Bun static characterization of NodeId-backed selected/focused/rename rendering.
- `test/rtl/issue-191.test.tsx`: explicit RTL adapter characterization for Escape cancellation.
- `../../../test/e2e/plasmon-file-entry-191.spec.ts`: packaged Playwright geometry regression using #187's `installPlasmonBrowserHealth`.

## Commands

```sh
bun test ./apps/plasmon/src/os/file-manager/issue-191.characterization.test.ts
bun test --preload ./apps/plasmon/test/setupHappyDom.ts ./apps/plasmon/test/rtl/issue-191.test.tsx
