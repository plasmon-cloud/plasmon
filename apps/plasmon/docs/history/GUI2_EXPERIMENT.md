# Plasmon GUI2 experiment

Branch: `version-0.1.0-gui2`

GUI2 is the second desktop-environment experiment for Plasmon. It starts from the working `version-0.1.0-gui` experiment (including its resilient Neutron app icon fallback) and deliberately pushes much closer to a polished browser desktop such as daedalOS while retaining Neutron's security boundary.
...
GUI2 includes lightweight Plasmon-native programs so the desktop can be exercised before Kernel floating windows exist:
...
The local GUI2 filesystem is intentionally an experiment backed by browser local storage. It is not a replacement for Neutron Files/VFS. The important product experiment is the UX: Atoms behave like file-like user objects that can be named, moved, opened, and eventually shared.
...
GUI2 never embeds a real Neutron app iframe inside the Plasmon iframe. Doing so would break the Kernel's immediate-parent/origin/private-MessagePort authentication model.
...
GUI2 mirrors the set of tray-capable apps exposed by `apps.describe` and displays them in its own system-tray flyout.
...
A Neutron app's desktop context menu includes `Download .neutron` because that is the right desktop affordance. Current Kernel APIs do not expose the original installed package archive, so GUI2 reports that limitation instead of fabricating a download. A future Kernel package-export tool would make this action functional.
...
The intended end state is not Plasmon permanently running a second window manager inside a Neutron tile. GUI2 is a UX prototype for a future generic Kernel floating-workspace mode where:
