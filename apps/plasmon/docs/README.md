...
Review fingerprints make implementation drift visible at those boundaries. [`DOCUMENTATION_REVIEW.md`](DOCUMENTATION_REVIEW.md) defines the `docs:review:status` / `docs:review` workflow, nearest-boundary ownership, and the conservative maintenance rule: every owned implementation change requires a substantive owning README or local AGENTS edit committed at or after the latest owned implementation commit before the marker can be refreshed. Uncommitted edits and marker-only commits do not satisfy the contract. The marker records deliberate review; it is not semantic proof that the documentation is correct.
