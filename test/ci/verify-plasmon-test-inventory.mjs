...
async function verify(inventory) {
  ...
  const taskbarContext402 = await readFile(resolve(repoRoot, 'test/e2e/plasmon-taskbar-context-menu.spec.ts'), 'utf8');
  assert(taskbarContext402.includes('test.describe.configure({ retries: 0 })'), 'taskbar geometry acceptance must remain retry-free');
  assert(taskbarContext402.includes('installPlasmonBrowserHealth'), 'taskbar geometry acceptance must install strict BrowserHealth');
  assert(taskbarContext402.includes('health.assertClean()'), 'taskbar geometry acceptance must assert strict BrowserHealth');
