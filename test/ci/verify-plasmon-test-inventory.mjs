...
async function verify(inventory) {
  ...
  assert(quarantineDoc.includes('No active `@r2-quarantine`'), 'Quarantine documentation must state that no active r2 browser quarantine remains');
  assert(quarantineDoc.includes('Restored required acceptance — #244'), 'Quarantine documentation must record #244 as restored to required execution');
  ...
  assert(quarantineDoc.includes('#250') && quarantineDoc.includes('#251'), 'Quarantine documentation must state that #250/#251 remain tracked while their tests stay required');
