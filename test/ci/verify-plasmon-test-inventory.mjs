...
function assertAlwaysRunPrWorkflow(source, label) {
  ...
  const forbidden = [
    ...
    'git diff --name-only', 'run_plasmon', 'plasmon_scope', 'Detect Plasmon-relevant changes',
  ];
  for (const fragment of forbidden) assert(!source.includes(fragment), `${label} must not select PR execution by changed files: ${fragment}`);
}
function scriptSpecPaths(script = '') { return sorted(script.match(/test\/e2e\/plasmon-[^\s"']+\.spec\.[cm]?[jt]sx?/g) ?? []); }
...
async function verify(inventory) {
  ...
  for (const [lane, paths] of Object.entries(browserLanes)) {
    for (const path of paths) assert(inventory.some((test) => test.path === path && test.layer === 'browser' && test.lane === lane), `${lane} owns missing browser test ${path}`);
  }
  ...
  for (const path of browserLanes.specialist) {
    ...
    if (expected) {
      ...
      for (const issue of expected.issues) assert(quarantineTags.some((tag) => tag.includes(issue)), `${path} active quarantine must remain linked to ${issue}`);
    } else {
  ...
  assert(demoGame.includes('{ tag: ["@issue-250", "@issue-123", "@issue-202", "@issue-64"] }'), 'Broad demo-game acceptance must remain required for #250/#123/#202/#64 without @r2-quarantine');
  assert(demoGame.includes('{ tag: ["@r2-quarantine", "@issue-124", "@issue-304"] }'), '#304 quarantine must remain isolated to the dedicated #124 saved-preview acceptance');
  assert(demoGame.includes('toHaveAttribute("src", /^blob:/'), '#304 executable debt must retain the required blob-backed preview assertion');
  ...
  for (const issue of ['#251', '#268', '#289', '#303', '#304', '#305', '#306', '#308']) assert(quarantineDoc.includes(issue), `Quarantine documentation must preserve ${issue} disposition`);
  ...
  for (const [label, workflow] of [['Packaged refactor smoke', smokeWorkflow], ['Packaged Playwright specialist acceptance', browserWorkflow], ['Packaged browser persistence', persistenceWorkflow]]) assertAlwaysRunPrWorkflow(workflow, label);
  ...
  for (const path of browserLanes.persistence) assert(persistenceWorkflow.includes(path), `Required Persistence CI must execute ${path}`);
  ...
  const counts = {
    fast: layerPaths(inventory, 'fast').length, rtl: layerPaths(inventory, 'rtl').length,
    package: layerPaths(inventory, 'package').length, browser: layerPaths(inventory, 'browser').length,
    nonPlasmonBrowser: layerPaths(inventory, 'non-plasmon-browser').length, excludedRed: layerPaths(inventory, 'excluded-red').length,
  };
