...
for (const gate of selected) {
  ...
  for (const fragment of [
    ...
    "Slow packaged/browser validation is deferred until approval and merge-queue entry.",
  ]) requireFragment(source, fragment, label);

  for (const fragment of ["pull_request_target", "continue-on-error: true", "    paths:", "    paths-ignore:"]) {
...
const demo = stepSection(browser.slice(browser.indexOf("  packaged-demo:")), "Package and run Plasmon demo browser acceptance");
...
]) requireFragment(demo, fragment, "Demo real slow gate");
if (/test\/e2e\/plasmon-[^\s'"\\]+\.spec\.[cm]?[jt]sx?/u.test(demo)) {
