...
Review follow-up removes all 20-second-or-larger synchronization waits from the changed browser journey. Taskbar and Monaco readiness use production-owned observable signals with a 10,000 ms fail-fast bound, while ordinary Explorer/window readiness uses Playwright defaults. This tightens timing without weakening the acceptance or changing Product semantics. PR #536 and ledger #295 carry the final exact-head gate disposition before merge.
