...
- narrowly scoped CI-verifier negative fixtures that reconstruct retired release-era selectors or labels solely to prove they remain forbidden; and
...
Line-scoped classifications are occurrence-checked: the verifier requires exactly one matching line in each declared file. If a classified occurrence disappears, duplicates, or moves, the classification check fails rather than silently broadening the exception.
