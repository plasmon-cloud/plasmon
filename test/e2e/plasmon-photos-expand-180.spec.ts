...
const GEOMETRY_STABLE_SAMPLES = 2;

type Geometry = { x: number; y: number; width: number; height: number };

function geometryMatches(actual: Geometry, expected: Geometry): boolean {
...
async function waitForSettledGeometry(
  actual: Locator,
  expected: Locator | Geometry,
): Promise<void> {
  let stableSamples = 0;
  await expect.poll(async () => {
    const actualBox = await actual.boundingBox();
    const expectedBox = "boundingBox" in expected ? await expected.boundingBox() : expected;
    const transitionRunning = await actual.evaluate((element) => (
      element.getAnimations().some((animation) => animation.playState !== "finished")
    ));

    if (!actualBox || !expectedBox || transitionRunning || !geometryMatches(actualBox, expectedBox)) {
      stableSamples = 0;
      return false;
    }

    stableSamples += 1;
    return stableSamples >= GEOMETRY_STABLE_SAMPLES;
...
test("#180 — packaged Photos expands inside Plasmon when browser fullscreen is denied", async ({ page }) => {
  ...
  try {
    ...
    // NativeWindow intentionally animates geometry. A single matching sample can
    // occur during the transition, so require the transition to finish and the
    // contract to hold across multiple polls before asserting the final bounds.
    await waitForSettledGeometry(photosWindow, windowLayer);
    ...
    await waitForSettledGeometry(photosWindow, floatingBefore);
