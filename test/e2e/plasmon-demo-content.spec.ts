...
test("plasmon:demo exposes demo files and Desktop shortcuts through installed Plasmon", async ({ page }) => {
  ...
  // Close the active topmost editor before the one behind it; the native
  // window layer intentionally prevents pointer events through the front window.
  ...
  await expect(documentsExplorer.locator("[data-fm-node-id]", { hasText: "Demo Notes.txt" }).first()).toBeVisible();
  await expect(documentsExplorer.locator("[data-fm-node-id]", { hasText: "Demo Guide.md" }).first()).toBeVisible();
