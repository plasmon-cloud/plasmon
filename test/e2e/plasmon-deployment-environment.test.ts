...
async function declaredArtifactPaths(manifestPath: string): Promise<string[]> {
  const manifest = JSON.parse(await readFile(resolve(repoRoot, manifestPath), "utf8")) as InlineManifest;
  return [manifest.artifacts.kernel.path, ...manifest.artifacts.packages.map((artifact) => artifact.path)];
}
...
describe("Plasmon deployment command semantics", () => {
  test("public local and demo scripts bind to explicit deployment scopes", async () => {
    ...
    for (const command of ["prepare", "serve", "reinstall", "status"]) {
      expect(packageJson.scripts[`plasmon:local:${command}`]).toBe(`bun test/e2e/plasmon-deployment-environment.ts local ${command}`);
      expect(packageJson.scripts[`plasmon:demo:${command}`]).toBe(`bun test/e2e/plasmon-deployment-environment.ts demo ${command}`);
    }
  });
  ...
  test("demo preparation resolves every artifact declared by plasmon.ndeploy.json", async () => {
    ...
    const actual = await resolveDeploymentArtifacts({ repoRoot, manifestPath: manifestForPlasmonDeployment("demo") });
    expect(actual.map((artifact) => artifact.archivePath)).toEqual(expected);
    expect(actual.map((artifact) => artifact.archivePath)).toContain("apps/plasmon/plasmon.v0.1.0.neutron");
  ...
  test("local preparation resolves only every artifact declared by plasmon-local.ndeploy.json", async () => {
    ...
    const actual = await resolveDeploymentArtifacts({ repoRoot, manifestPath: manifestForPlasmonDeployment("local") });
    expect(actual.map((artifact) => artifact.archivePath)).toEqual(expected);
  ...
  test("packaging deduplicates workspaces without dropping declared archives", () => {
    const artifacts = [
      { archivePath: "apps/example/a.neutron", workspace: "neutron-example", workspaceDirectory: resolve(repoRoot, "apps/example") },
      { archivePath: "apps/example/b.neutron", workspace: "neutron-example", workspaceDirectory: resolve(repoRoot, "apps/example") },
      { archivePath: "apps/other/c.neutron", workspace: "neutron-other", workspaceDirectory: resolve(repoRoot, "apps/other") },
    ];
    expect(artifacts.map((artifact) => artifact.archivePath)).toHaveLength(3);
    expect(workspacesToPackage(artifacts)).toEqual(["neutron-example", "neutron-other"]);
  });
});
