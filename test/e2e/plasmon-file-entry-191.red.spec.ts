...
/**
 * Issue #191 browser RED gate. This deliberately lives beside the canonical
 * packaged Playwright tests because Playwright's configured testDir is
 * test/e2e; it is not included by any ordinary smoke command.
 *
 * It consumes #187's browser-health ledger and packaged Plasmon environment.
 * It does not create a component harness or duplicate provisioning helpers.
 */
test("#191 RED — Desktop rename editor stays bounded by its FileEntry tile", async ({ page }) => {
  ...
  try {
    ...
    // The current renderer gives the desktop rename label a 260px expanded
    // width while the FileEntry tile is 92px wide. This is the intentional RED:
    // the editor must remain inside the entry/tile presentation and must not
    // become a second global label surface.
